...
export abstract class BaseController {
    ...
    public async preUpdateHook(model: IBaseModelDoc,request: Request, response: Response, next: NextFunction): Promise<IBaseModelDoc> {
        return model;
    }
    ...
    private async update(request: Request, response: Response, next: NextFunction, isFull: boolean): Promise<IBaseModelDoc> {
        try {
            if (await this.isModificationAllowed(request, response, next)) {
                // If something happens in the pre update hook, like a validation error, then we want to stop execution.
                let model = await this.preUpdateHook(this.repository.createFromBody(request.body), request,response,next);
                if(!model){ return; }
